...
public class EditRecipeController {
	...
	public void initialize() {
		...
		recipeNameField.setText(recipeEditing.getRecipeName());
		recipeDescriptionBox.setText(recipeEditing.getDescription());
		
		//Fill in ingredients table
		ObservableList<Ingredient> ingredientList = FXCollections.observableArrayList();
		for (int i = 0; i < recipeEditing.getIngredientList().getList().size(); i++) {
			ingredientList.add(recipeEditing.getIngredientList().getList().get(i));
		}
		...
		for (int i = 0; i<ingredientList.size(); i++) {
			ingredientTable.getItems().add(ingredientList.get(i));
		}
	}
	...
	public void saveRecipe(ActionEvent event) throws IOException {
		...
		for (int i =0; i<ingredientTable.getItems().size(); i++) {
			...
			if (ingredientTypeColumn.getCellObservableValue(i).getValue() != null) {
		...
		recipeEditing.setRecipeName("DeletedRecipe");
		RecipeApplication.recipeDatabase.addRecipe(newRecipe);
		newRecipe.setRecipeSteps(recipeEditing.getRecipeSteps());
		RecipeApplication.ingredientDatabase.addDatabaseIngredients();
	...
	public void recipeStepsWindow(Recipe newRecipe) throws IOException {
		EditStepsWindowController.setRecipe(newRecipe);
		Stage stage = new Stage();
		Parent newFXML = FXMLLoader.load(getClass().getResource("EditStepsWindow.fxml"));
	...
	public void deleteOriginalRecipe() throws IOException {
		Recipe recipeToDelete;
		for (int i = 0; i < RecipeApplication.recipeDatabase.getRecipeList().size(); i++) {
			if (recipeEditing == RecipeApplication.recipeDatabase.getRecipeList().get(i)) {
		...
		((FXMLDocumentController) loader.getController()).refreshRecipeDatabaseColumn();
	}
